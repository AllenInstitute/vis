...
export function decoderFactory(url: string, workerModule: URL, options?: CachingMultithreadedFetchStoreOptions) {
    ...
    const getSlice = async (metadata: OmeZarrMetadata, req: ZarrRequest, level: OmeZarrShapedDataset, signal?: AbortSignal) => {
        if (metadata.url !== url) {
            throw new Error('trying to use a decoder from a different store - we cant do that yet, although we could build a map of url->stores here if we wanted later - TODO')
        }
        ...
        return { shape, data: new Float32Array(data.buffer) };
