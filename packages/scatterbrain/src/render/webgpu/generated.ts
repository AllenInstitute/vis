import { difference, isEqual, keys, map } from "lodash";
...
export function buildPipeline(device: GPUDevice, config: Config) {
    ...
    // const lookupBindGroupLayout = pipeline.getBindGroupLayout(1);
    // const gradientBindGroupLayout = pipeline.getBindGroupLayout(2);
    // gradientBindGroupLayout.label = 'gradient bgLayout' // can I do that?

    // const bg0 = device.createBindGroup({
    //     layout: BGL,
    //     label: 'scatterplot uniform bind group',
    //     entries: [
    //         { binding: 0, resource: uniBuffer },

    //     ]
    // })
    // hmmm, if we want to re-create the texture... we'd need to build a new bindGroup...
    // that is annoying!
    ...
    const updateGradient = (data: Uint8Array<ArrayBuffer>) => {
        ...
        //     entries: [
        //         { binding: 2, resource: gradientTexture }
        //     ]
        // })
        endValidate(device);
