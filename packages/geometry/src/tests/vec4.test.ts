...
describe("Vec4", () => {
	...
	test("floor", () => {
		expect(Vec4.floor([33.999, -21.00001, 1, 0])).toStrictEqual([
			33, -22, 1, 0,
		]);
	});
	test("ceil", () => {
		expect(Vec4.ceil([2, 33.999, 0.001, -21.00001])).toStrictEqual([
			2, 34, 1, -21,
		]);
	});
	test("sum", () => {
	...
	test("map", () => {
		expect(Vec4.map([0, 0, 33, -2], (v, i) => (i === 2 ? v : i))).toStrictEqual(
			[0, 1, 33, 3],
		);
	});
	test("dot", () => {
	...
	test("finite", () => {
		...
		const infinityResult = Vec4.finite([Number.POSITIVE_INFINITY, 2, 3, 4]);
		expect(infinityResult).toBeFalsy();

		const nanResult = Vec4.finite([Number.NaN, 2, 3, 4]);
	...
	test("swizzle", () => {
		expect(Vec4.swizzle([111, 222, 33, 4], [3, 2, 1, 0])).toStrictEqual([
			4, 33, 222, 111,
		]);
		expect(Vec4.swizzle([111, 222, 33, 4], [0, 1, 3, 3])).toStrictEqual([
			111, 222, 4, 4,
		]);
		// dangerous cases:
		expect(Vec4.swizzle([111, 222, 33, 4], [2, 1, 3, 4])).toStrictEqual([
			33,
			222,
			4,
			undefined,
