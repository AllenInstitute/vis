...
describe("Box3D", () => {
	...
	test("map", () => {
		expect(
			Box3D.map(Box3D.create([0, 0, 0], [1, 1, 1]), (v) => Vec3.scale(v, 300)),
		).toStrictEqual(Box3D.create([0, 0, 0], [300, 300, 300]));
	});
	test("isValid", () => {
		const validBox = Box3D.isValid(box);
		expect(validBox).toBeTruthy();

		const nanBox = Box3D.isValid(
			Box3D.create([Number.NaN, Number.NaN, Number.NaN], [1, 1, 1]),
		);
	...
	test("intersection", () => {
		const intersection = Box3D.intersection(
			box,
			Box3D.create([2, 3, 4], [5, 5, 5]),
		);
		...
		const borderIntersection = Box3D.intersection(
			box,
			Box3D.create([1, 2, 3], [3, -4, 5]),
		);
		expect(borderIntersection).toBeUndefined();

		const noIntersection = Box3D.intersection(
			box,
			Box3D.create([10, 10, 10], [20, 20, 20]),
		);
