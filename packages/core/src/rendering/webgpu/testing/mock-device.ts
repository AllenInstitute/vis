...
export interface MockDevice {
    ...
    readonly passCommands: RecordedPassCommand[];
}
 ...
 *
 * Deliberately distinct from the renderer's own `PassCommand` (`renderer/encoder/pass-commands`):
 * this is a *recording* of what reached the mock pass, and it carries two extra members
 * (`beginRenderPass`, `endRenderPass`) that describe pass lifecycle rather than encodable
 * commands. The renderer type is not imported here — `testing` must not depend on `renderer`.
 */
export type RecordedPassCommand =
...
export function makeMockDevice(): MockDevice {
    ...
    const passCommands: RecordedPassCommand[] = [];
