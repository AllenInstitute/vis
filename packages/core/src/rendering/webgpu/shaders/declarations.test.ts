...
describe('constant', () => {
    it('generates const without a type annotation', () => {
        expect(constant('pi', 3.14159).gen()).toBe('const pi = 3.14159;');
    });

    it('generates const with an explicit type annotation', () => {
        expect(constant('pi', 3.14159, 'f32').gen()).toBe('const pi: f32 = 3.14159;');
    });

    it('generates const with a string initializer', () => {
        const init = 'array<vec2f,4>(vec2f(1,-1),vec2f(1,1),vec2f(-1,-1),vec2f(-1,1))';
        expect(constant('clip', init).gen()).toBe(`const clip = ${init};`);
    });
});

describe('override', () => {
    it('generates override with type only', () => {
        expect(override('scale', 'f32').gen()).toBe('var<override> scale: f32;');
    });

    it('generates override with initializer only', () => {
        expect(override('count', undefined, 10).gen()).toBe('var<override> count = 10;');
    });

    it('generates override with type and initializer', () => {
        expect(override('limit', 'u32', 256).gen()).toBe('var<override> limit: u32 = 256;');
    });

    it('generates override with an id attribute', () => {
        expect(override('idx', 'u32', undefined, [id(5)]).gen()).toBe('@id(5) var<override> idx: u32;');
    });
...
describe('privateVar', () => {
    it('generates var<private> with type only', () => {
        expect(privateVar('x', 'f32').gen()).toBe('var<private> x: f32;');
    });

    it('generates var<private> with type and initializer', () => {
        expect(privateVar('count', 'u32', 0).gen()).toBe('var<private> count: u32 = 0;');
    });

    it('generates var<private> with no type or initializer', () => {
        expect(privateVar('x').gen()).toBe('var<private> x;');
    });
});

describe('workgroupVar', () => {
    it('generates var<workgroup> with a type', () => {
        expect(workgroupVar('shared', 'array<f32, 64>').gen()).toBe('var<workgroup> shared: array<f32, 64>;');
    });
});

describe('uniform', () => {
    it('generates a uniform variable declaration', () => {
        expect(uniform('unis', 'Uniforms', 0, 0).gen()).toBe('@group(0) @binding(0) var<uniform> unis: Uniforms;');
    });

    it('generates with non-zero group and binding indices', () => {
        expect(uniform('data', 'DataBlock', 2, 5).gen()).toBe('@group(2) @binding(5) var<uniform> data: DataBlock;');
    });
});

describe('texture', () => {
    it('generates texture_2d<f32>', () => {
        expect(texture('colorMap', 'texture_2d<f32>', 0, 1).gen()).toBe(
            '@group(0) @binding(1) var colorMap: texture_2d<f32>;'
        );
    });

    it('generates texture_depth_2d', () => {
        expect(texture('depthMap', 'texture_depth_2d', 0, 2).gen()).toBe(
            '@group(0) @binding(2) var depthMap: texture_depth_2d;'
        );
    });

    it('generates texture_storage_2d with format and access', () => {
        expect(texture('outputTex', 'texture_storage_2d<rgba8unorm, write>', 0, 3).gen()).toBe(
...
describe('sampler', () => {
    it('generates a sampler declaration', () => {
        expect(sampler('s', 'sampler', 0, 3).gen()).toBe('@group(0) @binding(3) var s: sampler;');
    });

    it('generates a sampler_comparison declaration', () => {
        expect(sampler('sc', 'sampler_comparison', 0, 4).gen()).toBe(
...
describe('storage', () => {
    it('generates storage without an access mode', () => {
        expect(storage('buf', 'array<f32>', 0, 5).gen()).toBe('@group(0) @binding(5) var<storage> buf: array<f32>;');
    });

    it('generates storage with read access mode', () => {
        expect(storage('buf', 'array<f32>', 0, 5, 'read').gen()).toBe(
            '@group(0) @binding(5) var<storage, read> buf: array<f32>;'
        );
    });

    it('generates storage with read_write access mode', () => {
        expect(storage('buf', 'array<f32>', 0, 5, 'read_write').gen()).toBe(
...
describe('member', () => {
    it('generates a plain struct member', () => {
        expect(member('x', 'f32').gen()).toBe('x: f32');
    });

    it('generates a member with a builtin attribute', () => {
        expect(member('pos', 'vec4f', [builtin('position')]).gen()).toBe('@builtin(position) pos: vec4f');
    });

    it('generates a member with a location attribute', () => {
        expect(member('color', 'vec4f', [location(0)]).gen()).toBe('@location(0) color: vec4f');
    });

    it('generates a member with align and size attributes', () => {
        expect(member('data', 'vec4f', [align(16), size(16)]).gen()).toBe('@align(16) @size(16) data: vec4f');
    });
});

describe('struct', () => {
    it('generates a struct with a single field', () => {
        expect(struct('Point', [member('x', 'f32')]).gen()).toBe('struct Point { x: f32 }');
    });

    it('generates a struct with multiple fields', () => {
        expect(struct('Vec2', [member('x', 'f32'), member('y', 'f32')]).gen()).toBe('struct Vec2 { x: f32, y: f32 }');
    });
    ...
    it('generates a struct with attributed members', () => {
        ...
        expect(s.gen()).toBe('struct VsOutput { @builtin(position) pos: vec4f, @location(0) color: vec4f }');
    });

    it('has identType of "struct"', () => {
        expect(struct('Test', [member('x', 'f32')]).identType).toBe('struct');
    });
});

describe('alias', () => {
    it('generates alias with a string type', () => {
        expect(alias('Vec2', 'vec2f').gen()).toBe('alias Vec2 = vec2f;');
    });

    it('generates alias referencing a struct by name', () => {
        const s = struct('Point', [member('x', 'f32')]);
        expect(alias('PointAlias', s).gen()).toBe('alias PointAlias = Point;');
    });

    it('generates alias referencing another alias by name', () => {
        const inner = alias('Base', 'vec2f');
        expect(alias('Derived', inner).gen()).toBe('alias Derived = Base;');
    });

    it('has identType of "alias"', () => {
        expect(alias('A', 'f32').identType).toBe('alias');
    });
});

describe('param', () => {
    it('generates a plain parameter', () => {
        expect(param('x', 'f32').gen()).toBe('x: f32');
    });

    it('generates a parameter with a builtin attribute', () => {
        expect(param('idx', 'u32', [builtin('vertex_index')]).gen()).toBe('@builtin(vertex_index) idx: u32');
    });
});

describe('returns', () => {
    it('generates a plain return type', () => {
        expect(returns('f32').gen()).toBe('f32');
    });

    it('generates a plain return type from a Struct', () => {
        const s = struct('Point', [member('x', 'f32')]);
        expect(returns(s).gen()).toBe('Point');
    });

    it('generates a plain return type from an aliased Struct', () => {
        const s = struct('Point', [member('x', 'f32')]);
        const a = alias('PointAlias', s);
        expect(returns(a).gen()).toBe('PointAlias');
    });

    it('generates a return type with a location attribute', () => {
        expect(returns('vec4f', [location(0)]).gen()).toBe('@location(0) vec4f');
    });
});
...
describe('fragmentEntry', () => {
    it('generates a @fragment-annotated function', () => {
        ...
        expect(f.gen()).toBe('@fragment fn fmain(v: VsOutput) -> @location(0) vec4f { return v.color; }');
    });

    it('has @fragment as its sole function attribute', () => {
        const f = fragmentEntry('fmain', [], () => '');
        expect(f.attributes).toHaveLength(1);
        expect(f.attributes?.[0]?.gen()).toBe('@fragment');
    });
});
...
describe('computeEntry', () => {
    it('generates a @compute-annotated function', () => {
        ...
        expect(f.gen()).toBe('@compute fn cmain(@builtin(global_invocation_id) gid: vec3u) {  }');
    });

    it('has @compute as its sole function attribute', () => {
        const f = computeEntry('cmain', [], () => '');
        expect(f.attributes).toHaveLength(1);
        expect(f.attributes?.[0]?.gen()).toBe('@compute');
    });
});
