...
describe("box2D", () => {
	...
	test("map", () => {
		expect(
			Box2D.map(Box2D.create([0, 0], [1, 1]), (v) => Vec2.scale(v, 200)),
		).toStrictEqual(Box2D.create([0, 0], [200, 200]));
	});
	test("isValid", () => {
		const validBox = Box2D.isValid(box);
		expect(validBox).toBeTruthy();

		const nanBox = Box2D.isValid(
			Box2D.create([Number.NaN, Number.NaN], [1, 1]),
		);
		expect(nanBox).toBeFalsy();

		const infinityBox = Box2D.isValid(
			Box2D.create(
	...
	test("intersection", () => {
		...
		const borderIntersection = Box2D.intersection(
			box,
			Box2D.create([3, 3], [5, 5]),
		);
		expect(borderIntersection).toBeUndefined();

		const noIntersection = Box2D.intersection(
			box,
			Box2D.create([10, 10], [20, 20]),
		);
