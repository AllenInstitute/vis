 ...
 */
function renderIfCached(mutableCache, item, settings, abort, render, requestsForItem, cacheKeyForRequest) {
    const requests = requestsForItem(item, settings, abort);
    const requestKeys = Object.keys(requests);
    if (mutableCache.areKeysAllCached(requestKeys.map((req) => cacheKeyForRequest(req, item, settings)))) {
        render(item, settings, mapify(requestKeys.map((key) => ({ key, result: mutableCache.getCached(cacheKeyForRequest(key, item, settings)) }))));
        return true;
    }
    return requests;
}
 ...
 */
function cacheAndRenderItem(mutableCache, item, settings, abort, requestsForItem, render, handleError, cacheKeyForRequest) {
    // although this doesnt make  a ton of sense - lets check the abort signal anyway?
    if (abort.aborted)
        return false;
    const didRenderOrRequests = renderIfCached(mutableCache, item, settings, abort, render, requestsForItem, cacheKeyForRequest);
    if (didRenderOrRequests !== true) {
        // an async request(s) will need to happen first:
        const requests = didRenderOrRequests;
        const allDataReady = Promise.all(Object.entries(requests).map(([key, promises]) => promisify(mutableCache.cache(cacheKeyForRequest(key, item, settings), promises)).then((result) => ({
            key,
            result,
        }))));
        return allDataReady
 ...
 */
export function beginLongRunningFrame(maximumInflightAsyncTasks, queueProcessingIntervalMS, items, mutableCache, settings, requestsForItem, render, lifecycleCallback, cacheKeyForRequest = (key) => key) {
    ...
    const doWorkOnQueue = (intervalId) => {
        ...
        while (inFlight.size < Math.max(maximumInflightAsyncTasks, 1)) {
            ...
            try {
                const result = cacheAndRenderItem(mutableCache, itemToRender, settings, abort.signal, requestsForItem, render, cleanupOnError, cacheKeyForRequest);
