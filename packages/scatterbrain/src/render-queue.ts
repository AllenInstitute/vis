...
function renderIfCached<Column, Item, Settings>(
  ...
  if (
    ...
    render(
      item,
      settings,
      mapify(reqs.map((key) => ({ key, result: mutableCache.getCached(cacheKeyForRequest(key, item, settings)) })))
    );
    return true;
  }
  return requests;
...
function cacheAndRenderItem<Column, Item, Settings>(
  ...
  if (didRenderOrRequests !== true) {
    ...
    const allDataReady = Promise.all(
      reqs.map((key) =>
        promisify(mutableCache.cache(cacheKeyForRequest(key, item, settings), requests[key])).then((result) => ({
          key,
          result,
        }))
      )
...
export function beginLongRunningFrame<Column, Item, Settings>(
  ...
  items.forEach((item) => {
    if (
      ...
      renderIfCached(mutableCache, item, settings, abort.signal, render, requestsForItem, cacheKeyForRequest) !== true
    ) {
      queue.push(item);
