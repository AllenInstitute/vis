...
interface OmeZarrV3ViewerProps {
...
export function OmeZarrV3Viewer({
    omezarr,
    id,
    settings,
    onWheel,
    ...
    onMouseMove,
    onMouseLeave,
}: OmeZarrV3ViewerProps) {
    const canvas = useRef<HTMLCanvasElement>(null);
    const server = useContext(renderServerContext);
    const renderer = useRef<ReturnType<typeof buildAsyncOmeZarrPlanarRenderer>>(undefined);
    ...
    useEffect(() => {
        ...
        if (server?.regl && omezarr) {
            const numChannels = omezarr.getColorChannels().length || 3;
            renderer.current = buildAsyncOmeZarrPlanarRenderer(server.regl, defaultPlanarDecoder, {
                numChannels,
    ...
    useEffect(() => {
        ...
        if (server && renderer.current && canvas.current && omezarr) {
            const renderFrame: RenderFrameFn<OmeZarrFileset, PlanarVoxelTile> = (target, cache, callback) => {
                if (renderer.current) {
            ...
            const lowResPreview: RenderFrameFn<OmeZarrFileset, PlanarVoxelTile> = (target, cache, callback) => {
                if (renderer.current) {
                    // if we had a stashed buffer of the previous frame...
                    // we could pre-load it into target, right here!
