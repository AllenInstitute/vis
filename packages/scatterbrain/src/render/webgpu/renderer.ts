...
export function buildRenderFrameFn(device: GPUDevice, settings: ShaderSettings) {
    ...
    const toGpuBuffer = (buffer: ArrayBuffer, type: WebGLSafeBasicType) => {
        ...
        const B = device.createBuffer({ size: buffer.byteLength, usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.VERTEX });
    ...
    const render = (props: RenderPassProps & { client: ReturnType<typeof buildScatterbrainCacheClient<VBO>> }) => {
        ...
        const bg0 = updateUniforms({ ...omit(uniforms, 'camera', 'spatialFilterBox', 'quantitativeRangeFilters'), view: Box2D.toFlatArray(uniforms.camera.view), spatialFilterBox: Box2D.toFlatArray(uniforms.spatialFilterBox), ...uniforms.quantitativeRangeFilters });
