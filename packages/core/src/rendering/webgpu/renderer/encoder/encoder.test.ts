...
function passCmds(m: MockDevice, kind: RecordedPassCommand['kind']): RecordedPassCommand[] {
...
describe('GraphEncoder — scoped state restoration', () => {
    it('viewport subtree restores the prior viewport on exit', () => {
        ...
        const viewports = passCmds(m, 'setViewport').filter(
            (c): c is Extract<RecordedPassCommand, { kind: 'setViewport' }> => c.kind === 'setViewport'
        );
